...
_CATALOG_HELP = (
    ...
    "'mock://incidents' dev source; optional props.title, props.columns as string[]; OR props.rows: "
    "a list of row objects you computed yourself — each row a flat {column: scalar} map (string/"
    "number/boolean/null values only; NO nested objects or arrays — flatten those into their own "
    "columns); optional props.schema to type the columns: a list of {name, type} objects, e.g. "
    "[{\"name\":\"vessel\",\"type\":\"string\"},{\"name\":\"days\",\"type\":\"number\"}] — NOT an "
    "object keyed by column name and NOT a bare list of column names; max "
    "50 rows — bindings.source wins when both are present). Grid: layout.cols (default 12); every "
    "TOP-LEVEL component "
    ...
    "href — NOT a STAC item or collection link, and NOT an s3:// URI (a browser cannot "
    "fetch those). "
    "OPTICAL ONLY for now: plot the 'visual'/TCI true-colour asset. The open catalogs "
    "publish SAR (Sentinel-1 GRD) as s3:// requester-pays assets that a browser cannot "
    "fetch, so do NOT author SAR scenes in the `imagery` block — they will fail to load. "
    "You may still RECOMMEND SAR tasking in prose or a table; just do not try to display "
    "it. (If a verified public HTTPS SAR COG ever exists, sensor:'sar' still renders it "
    "with the contrast stretch a single-band raster needs — without which it would paint "
    "a BLACK RECTANGLE.) `sensor` is REQUIRED and must be correct. `bbox` is WGS84 lon-first "
    "[minLon,minLat,maxLon,maxLat]. Imagery is a BACKDROP: it is not selectable and "
    "carries no rows — keep the AIS/vector layers in bindings.layers as the evidence, "
    "with imagery underneath. ALSO author an esri:layer-list so the user can toggle "
    "scenes on and off."
...
RENDER_VIEW_SCHEMA = {
    ...
    "description": (
        ...
        "Hero a map at layer:'base' only when the geography IS the answer. Author at most "
