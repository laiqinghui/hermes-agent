...
_CATALOG_HELP = (
    ...
    "SAR) — NOT a STAC item or collection link, and NOT an s3:// URI (a browser cannot "
    "fetch those; Earth Search publishes Sentinel-1 GRD that way). `sensor` is REQUIRED "
    "and must be correct: a SAR scene marked 'optical' renders as a BLACK RECTANGLE "
    "because it needs a contrast stretch. `bbox` is WGS84 lon-first "
